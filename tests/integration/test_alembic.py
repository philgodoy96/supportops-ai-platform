...
SENSITIVE_EXECUTION_GRANT_REVISION = "e4a7c9d2f1b6"
...
async def test_alembic_upgrade_creates_business_tables(
    ...
    try:
        ...
        assert await relation_exists(engine, "public.ticket_escalations")
...
async def test_alembic_ticket_escalation_migration_upgrades_and_downgrades(
    exclusive_integration_database: None,
) -> None:
    settings = Settings()
    engine = create_async_engine(str(settings.postgresql_url))
    ...
    downgrade_prior = run_alembic_command(
        "downgrade",
        SENSITIVE_EXECUTION_GRANT_REVISION,
    )
    assert downgrade_prior.returncode == 0, downgrade_prior.stderr
    assert not await relation_exists(
        engine,
    ...
    try:
        async with engine.begin() as connection:
            await connection.execute(
                text(
                    ...
                    INSERT INTO workspaces (
                        ...
                        'escalation-workspace',
                        TIMESTAMPTZ '2026-08-03 18:30:00+00',
                        TIMESTAMPTZ '2026-08-03 18:30:00+00'
                    )
                    """
                ),
            ...
            await connection.execute(
                text(
                    ...
                    INSERT INTO agent_runs (
                        ...
                        'worker-a',
                        :lease_token,
                        TIMESTAMPTZ '2026-08-03 18:40:00+00',
                        TIMESTAMPTZ '2026-08-03 18:31:00+00',
                        NULL,
                        NULL,
                        NULL,
                        :ingestion_request_id,
            ...
            await connection.execute(
                text(
                    ...
                    INSERT INTO llm_invocations (
                        ...
                        12000,
                        'llm_timeout',
                        TIMESTAMPTZ '2026-08-03 18:32:00+00'
                    )
                    """
                ),
                {
            ...
            for call_id, sequence, provider_id, fp in (
                (tool_call_id, 1, "provider-1", fingerprint),
                (second_tool_call_id, 2, "provider-2", "b" * 64),
            ):
                await connection.execute(
                    text(
                        """
                        INSERT INTO agent_tool_calls (
                            ...
                            NULL,
                            NULL,
                            TIMESTAMPTZ '2026-08-03 18:31:01+00',
                            NULL,
                            NULL
                        )
                        """
            ...
            for approval, call_id, fp in (
                (approval_id, tool_call_id, fingerprint),
                (second_approval_id, second_tool_call_id, "b" * 64),
            ):
                await connection.execute(
                    text(
                        """
                        INSERT INTO approval_requests (
                            ...
                            TIMESTAMPTZ '2026-08-04 18:30:00+00',
                            'operator:alice',
                            NULL,
                            :decision_request_id,
                            :decision_correlation_id,
                            TIMESTAMPTZ '2026-08-03 18:35:00+00',
                            TIMESTAMPTZ '2026-08-03 18:33:00+00',
                            TIMESTAMPTZ '2026-08-03 18:35:00+00'
                        )
                        """
                    ),
            ...
            await connection.execute(
                text(
                    ...
                    INSERT INTO ticket_escalations (
                        ...
                        :tool_call_id,
                        'engineering_support',
                        'A product defect requires review.',
                        TIMESTAMPTZ '2026-08-03 18:37:00+00'
                    )
                    """
                ),
                {
                ...
                await connection.execute(
                    text(
                        ...
                        INSERT INTO ticket_escalations (
                            ...
                            :tool_call_id,
                            'engineering_support',
                            'Duplicate approval escalation.',
                            TIMESTAMPTZ '2026-08-03 18:38:00+00'
                        )
                        """
                    ),
                    {
                ...
                await connection.execute(
                    text(
                        ...
                        INSERT INTO ticket_escalations (
                            ...
                            :tool_call_id,
                            'engineering_support',
                            'Workspace ID uniqueness conflict.',
                            TIMESTAMPTZ '2026-08-03 18:38:00+00'
                        )
                        """
                    ),
        ...
        column_names = {column.name for column in table.c}
        assert "executed_by_agent_run_attempt_id" in column_names
        assert "target_queue" in column_names
        assert "reason" in column_names
        constraint_names = {constraint.name for constraint in table.constraints}
        assert "ck_ticket_escalations_target_queue_format" in constraint_names
        assert "uq_ticket_escalations_approval_request" in constraint_names
        assert "fk_ticket_escalations_execution_attempt" in constraint_names
        index_names = {index.name for index in table.indexes}
        ...
        check = run_alembic_command("check")
