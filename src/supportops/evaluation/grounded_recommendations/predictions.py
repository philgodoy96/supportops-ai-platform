...
GroundedRecommendationPrediction = EvaluationPredictionEnvelope[
    GroundedRecommendationPredictionPayload
]

_PREDICTION_ADAPTER = TypeAdapter(GroundedRecommendationPrediction)


class GroundedRecommendationPredictionError(ValueError):
    """Raised when grounded recommendation predictions are invalid."""


def load_grounded_recommendation_predictions(
    path: Path,
) -> tuple[tuple[GroundedRecommendationPrediction, ...], str]:
    """Load, validate, and hash canonical grounded prediction JSONL."""

    predictions: list[GroundedRecommendationPrediction] = []
    ...
    with path.open("r", encoding="utf-8") as prediction_file:
        for line_number, raw_line in enumerate(
            ...
            try:
                ...
                raise GroundedRecommendationPredictionError(
    ...
    if not predictions:
        raise GroundedRecommendationPredictionError("prediction set must not be empty")
